...
export default function PostCard({ post, hideShare = false, onOpenReactionModal }: PostCardProps) {
  ...
  // Hàm tạo màu ngẫu nhiên cho tag dựa trên nội dung
        ...
        {post.tags && post.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-4">
            {post.tags.slice(0, 3).map((tag, index) => {
              ...
              return (
                <span key={index} className={`px-2 py-1 ${colors.bg} ${colors.text} text-xs rounded-md flex items-center gap-1 font-medium`}>
