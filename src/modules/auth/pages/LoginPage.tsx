...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (searchParams.get('sessionExpired') === 'true') {
      toast.error('Phiên đăng nhập đã hết hạn hoặc bạn đã đăng nhập trên thiết bị khác. Vui lòng đăng nhập lại!', {
        duration: 5000,
        position: 'top-center',
      });
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const err = error as { response?: { data?: { locked?: boolean; alreadyLoggedIn?: boolean; message?: string } } };
      if (err.response?.data?.locked) {
        toast.error('Bạn đã bị khóa tài khoản!', {
          duration: 6000,
          position: 'top-right',
        });
      } else if (err.response?.data?.alreadyLoggedIn) {
        toast.error('Tài khoản đang được đăng nhập trên thiết bị khác. Vui lòng đăng xuất thiết bị đó trước!', {
          duration: 6000,
          position: 'top-right',
        });
      } else {
      ...
      <div className="max-w-md w-full mx-4 relative z-10">
        <div className="bg-white backdrop-blur-sm rounded-3xl shadow-2xl border border-white/20 p-8">
          ...
          <p className="text-center text-sm text-gray-600 mt-6">
            Chưa có tài khoản? 
            <Link to="/register" className="text-blue-600 hover:text-blue-700 font-semibold ml-1 transition-all duration-300 hover:underline underline-offset-4 decoration-2">
              Đăng ký ngay
            </Link>
          </p>
        </div>
